...
document.getElementById("startup").addEventListener("click",()=>{
  ...
  person=name.value;
  conversation=loadHistory(person);
  document.getElementById("name").style.display="none";
  document.getElementById("startup").style.display="none";
  const chatO=document.getElementById("chatOverview");
  const beginG=beginning();
  const output=`${person}, ${beginG}`;
  chatO.innerHTML=conversation.join("<br>")+"<br>"+output;
  setTimeout(noResponse,10000);
});
// Event listener for chatting
document.getElementById("enter").addEventListener("click",()=>{
  const chat=document.getElementById("chat");
  const chatI=chat.value;
  // To clear the state of the application
  if(chatI==='/clear'){
...
function saveHistory(person,conversation){
  history[person]=conversation;
  localStorage.setItem(person,JSON.stringify(history));
}
//To load the previous conversation of a person
function loadHistory(person){
   let conversation_arr=[];
    const conversationHist=localStorage.getItem(person);
  if (conversationHist) {
    conversation_arr.push(conversationHist)
...
//To check if visited the dictionary for a keyword
...
function respond(){
    ...
    for(const i of dict.entries){
        for(const j of i.key){
            if(chatI.includes(j)){
              if(!visited[j]){
                visited[j]=new Set();
                visited_ques[j] = new Set();
              }
              if(visited[j].size===i.answer.length){
                visited[j].clear();
              }
              if(visited_ques[j].size===i.question.length)
              {
                visited_ques[j].clear();
              }
              let vis;
              //Check until we find the non-visited response
              do{
                vis=getVisited(i);
              } while(visited[j].has(vis.answer) || visited[j].has(vis.question));
              visited[j].add(vis.answer);
              visited_ques[j].add(vis.question);
              //Create the output response
              output=`${person}, ${vis.answer} ${vis.question}`;
              break;
            }
        }
    }
    ...
    if(output===""){
        ...
        output=`${person}, ${noOutput[random]}`;
    }
    const personH=`${person}: ${chat.value}`;
    const elizaH=`Eliza: ${output}`;
    conversation.push(personH,elizaH);
    chat.value="";
    chatO.innerHTML=conversation.join("<br>");
    saveHistory(person,conversation);
    setTimeout(noResponse,10000);
}
